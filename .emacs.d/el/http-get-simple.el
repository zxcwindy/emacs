;;; http-get-simple.el --- HTTP GET requests using the url library
...
;; Provides ways to use the url library to perform HTTP GET requests.
;; See the documentation to `http-get-simple' for more information.
;;
;; The `url-http' library does not handle 1xx response codes.

;; However, as RFC 2616 puts it:
;;     a server MAY send a 100 (Continue)
;;     status in response to an HTTP/1.1 PUT or GET request that does
...
;; `http-get-finesse-code-100'. Note that the latter advises
;; 'url-http-parse-response'
;;
;;; Change Log:

;; 11/06/2008 Set `url-http-version' to "1.0" when geting.
...
(defun http-get-simple (url fields &optional charset)
  "Send FIELDS to URL as an HTTP GET request, returning the response
and response headers.
FIELDS is an alist, eg ((field-name . \"value\")); all values
need to be strings, and they are encoded using CHARSET,
which defaults to 'utf-8"
  (http-get-simple-internal
   url
   (http-get-encode-fields fields charset)
   charset
   `(("Content-Type"
      .
...
(defun http-get-simple-multipart (url fields files &optional charset)
  "Send FIELDS and FILES to URL as a multipart HTTP GET, returning the
response and response headers.
FIELDS is an alist, as for `http-get-simple', FILES is an a list of
\(fieldname \"filename\" \"file MIME type\" \"file data\")*"
(let ((boundary (http-get-multipart-boundary)))
  (http-get-simple-internal
   url
   (http-get-encode-multipart-data fields files charset)
   charset
   `(("Content-Type"
      .
      ,(http-get-content-type
...
(defun http-get-content-type (content-type &optional charset)
  (if charset
      (format "%s; charset=%s" content-type (http-get-charset-name charset))
      content-type))


(defun http-get-charset-name (charset)
  (symbol-name charset))


;; based on `http-url-encode' from the from http-get package
;; http://savannah.nongnu.org/projects/http-emacs
(defun http-get-encode-string (str content-type)
...
(defun http-get-encode-fields (fields &optional charset)
  "Encode FIELDS using `http-get-encode-string', where
    ...
    (mapconcat #'identity
	       (mapcar '(lambda (field)
			 (concat (symbol-name (car field))
			  "="
			  (http-get-encode-string (cdr field) charset)))
		       (mapcan '(lambda (field)
...
(defun http-get-simple-internal (url data charset extra-headers)
  (let ((url-request-method        "GET")
	(url-request-data          data)
	(url-request-extra-headers extra-headers)
        (url-mime-charset-string   (http-get-charset-name charset)))
    (let (header
	  data
	  status)
      (with-current-buffer
...
(defun http-get-bound-field (&rest parts)
  (let ((boundary (format "--%s" (http-get-multipart-boundary))))
    (http-get-join-lines  boundary parts)))


(defun http-get-encode-multipart-data (fields files charset)
  "Return FIELDS and FILES encoded for use as the data for a multipart HTTP GET request"
  (http-get-join-lines
   (mapcar '(lambda (field)
	     (http-get-bound-field
   ...
   (mapcan '(lambda (file)
		...
		(format "Content-type: %s" (http-get-content-type mime-type charset))
		""
		data)))
	   files)
   (format "--%s--" (http-get-multipart-boundary))))


(defun http-get-join-lines (&rest bits)
  (let ((sep "\r\n"))
    (mapconcat (lambda (bit)
		 (if (listp bit)
		     (apply 'http-get-join-lines bit)
		     bit))
	       bits sep)))


(defun http-get-finesse-code-100 ()
  "Transforms response code 100 into 200, to avoid errors when the
server sends code 100 in response to a GET request."
  (defadvice url-http-parse-response (after url-http-parse-response-100 activate)
    ...
    (when (and (= 100               url-http-response-status)
               (string-equal "GET" url-request-method)
               (string-equal "1.1"  url-http-version))
      (setf url-http-response-status 200))))


(provide 'http-get-simple)
;;; http-get-simple.el ends here
